...
def ranking_clustering(
    K, ranking_set, max_iters, repeats, mode_ranking_weight, random_state 
):
    """
    Clusters ranking_set in K clusters using a Kmeans++ initialization and several repeats
    """
    centroids, ranking_centroid_distances, mode_rank = initialization(ranking_set, K, seed=random_state)
    best_score = -1.1
    best_centroids = []
    assigned_centroids = []
    scores = []
    for r in range(repeats):
        scores_r = []
        centroids, ranking_centroid_distances, mode_rank = initialization(ranking_set, K, seed=random_state + r)
...
def initialization(incomplete_ranking_set, K, seed=0):
    """
    ...
    centroids = []
    centroids.append(random_first_rank_initialization(incomplete_ranking_set, mode_rank, seed))
...
def random_first_rank_initialization(ranking_set, mode, seed=0):
    random.seed(seed)
    first_centroid_candidate = random.choice(ranking_set)
...
def farther_ranking_sample(incomplete_ranking_set, distances):
    ...
    return incomplete_ranking_set[np.random.choice(np.arange(len(incomplete_ranking_set)), p=probs)]
