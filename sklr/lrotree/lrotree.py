...
class LROTree(BaseEstimator, metaclass=ABCMeta):
    ...
    def __init__(self, 
                 ...
                 clustering_balance_assign = False,
        ...
        self.clustering_balance_assign = clustering_balance_assign
        ...
        skrandom.set_random_seed(random_seed)
    ...
    def fit(self, X, Y):
        ...
        builder = LROTreeBuilder(
                 ...
                 clustering_balance_assign = self.clustering_balance_assign,
...
class LROTreeBuilder:
    ...
    def _build_internal(self, tree, X, Y, depth):
        ...
        best_centroids, assigned_centroids, best_score, scores = ranking_clustering(
            ...
            self.clustering_balance_assign,
        )

        if np.unique(assigned_centroids).size == 1:
            # Although very rare, centroids could end up beeing the same if there are missing labels 
            # that after completion end up being the same (I think). Just construct a leaf node
